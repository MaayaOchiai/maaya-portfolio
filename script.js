...
async function fetchNoteArticles() {
    ...
    const API_URL = 'https://note.com/api/v2/creators/maaya_frescobol/contents?page=1';
    ...
    try {
        ...
        grid.innerHTML = items.slice(0, 3).map(item => {
            const date = new Date(item.publishAt).toLocaleDateString('ja-JP', {
                year: 'numeric', month: 'long', day: 'numeric'
            });
            const thumb = item.eyecatch || item.spEyecatch;
            const thumbHtml = thumb
                ? `<img class="note-card-thumb" src="${thumb}" alt="${escapeHtml(item.name)}" loading="lazy">`
                : `<div class="note-card-thumb-placeholder">note</div>`;
            ...
            <div class="note-card">
                ...
                <div class="note-card-body">
                    ...
                    <a class="note-card-title" href="${item.noteUrl}" target="_blank" rel="noopener noreferrer">
        ...
        <p style="text-align:center;color:var(--text-muted);grid-column:1/-1;padding:40px 0;">
            記事の読み込みに失敗しました。
            <a href="https://note.com/maaya_frescobol" target="_blank" rel="noopener noreferrer">noteで直接ご覧ください →</a>
        </p>`;
    }
}
